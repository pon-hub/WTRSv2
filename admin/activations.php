...
<main class="main-content">
  ...
  <?php if ($flash): ?>
    <div id="flash-message"
      style="margin-bottom: 2rem; padding: 1rem 1.5rem; border-radius: var(--radius-sm); font-weight:600; font-size: 0.85rem; color: #fff; background: <?= $flash['type'] === 'success' ? '#065F46' : '#991B1B' ?>; transition: opacity 0.5s ease-out;">
      <i class="ph-bold <?= $flash['type'] === 'success' ? 'ph-check-circle' : 'ph-warning-circle' ?>"
        style="margin-right:0.5rem; font-size:1.1rem; vertical-align:middle;"></i>
  ...
  <div style="display: grid; grid-template-columns: 3fr 1fr; gap: 2rem;">
                ...
                <td>
                  <div class="submitter-cell">
                    ...
                    <div>
                      ...
                      <div class="submitter-college" style="font-weight:700; color:var(--text-dark); margin-top:0.2rem;">
                        <?= htmlspecialchars($pu['email']) ?></div>
                    </div>
                  </div>
                </td>
                <td>
                ...
                <td style="text-align: right;">
                  <form method="post" style="display:inline-block;">
                    ...
                    <button type="submit" class="btn btn-primary"
                      style="padding:0.35rem 0.75rem; font-size:0.75rem;">Activate</button>
                  </form>
                  <form method="post" style="display:inline-block; margin-left:0.25rem;">
                    <input type="hidden" name="user_id" value="<?= intval($pu['id']) ?>">
                    <input type="hidden" name="action" value="reject">
                    <button type="submit" class="btn btn-secondary"
                      style="padding:0.35rem 0.75rem; font-size:0.75rem; color:#DC2626; border-color:#DC2626;">Reject</button>
                  </form>
                </td>
              </tr>
            <?php endforeach; ?>
    ...
    <div>
      ...
      <div
        style="background: var(--surface); border-left: 4px solid var(--crimson); padding: 1.5rem; border-radius: 0 var(--radius-sm) var(--radius-sm) 0; box-shadow: var(--shadow-sm);">
        <div
          style="font-size: 0.7rem; font-weight: 800; letter-spacing: 0.1em; text-transform: uppercase; color: var(--crimson); margin-bottom: 0.75rem; display: flex; align-items: center; gap: 0.4rem;">
        ...
        <p style="font-family: 'Georgia', serif; font-size: 0.95rem; line-height: 1.6; color: var(--text-dark);">
          Account activations must strictly verify that students are actively enrolled and using their @wmsu.edu.ph
