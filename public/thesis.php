...
$stmt = $pdo->prepare("SELECT t.*, 
                       ...
                       WHERE t.thesis_code = :code AND t.status = 'approved'");
...
// Fetch recommended (other approved theses)
$recStmt = $pdo->prepare("SELECT t.thesis_code, t.title, u.college 
                          FROM theses t 
                          JOIN users u ON t.author_id = u.id
                          WHERE t.status = 'approved' AND t.id != :id 
  ...
  <main class="thesis-page-container">
    ...
    <article class="digital-sheet">
       ...
       <div class="manuscript-link-card">
          ...
          <a href="<?= BASE_URL ?>public/uploads/<?= htmlspecialchars($versions[0]['file_path'] ?? '') ?>" target="_blank" class="btn btn-primary">
      ...
      <div class="recommended-grid">
        <?php foreach ($recommended as $rec): ?>
          <article class="rec-card">
            ...
            <a href="<?= BASE_URL ?>public/thesis.php?code=<?= htmlspecialchars($rec['thesis_code']) ?>" style="font-size: 0.75rem; font-weight: 800; color: var(--crimson); text-decoration: none; display: flex; align-items: center; gap: 0.4rem;">
