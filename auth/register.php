...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!$first_name || !$last_name || !$email || !$password || !$confirm_password || !$college) {
    ...
    if (!$error) {
      ...
      $stmt = $pdo->prepare('INSERT INTO users (first_name, last_name, email, password, role, student_id, college, course, year_level, status) VALUES (:first_name, :last_name, :email, :password, :role, :student_id, :college, :course, :year_level, :status)');
      $stmt->execute([
